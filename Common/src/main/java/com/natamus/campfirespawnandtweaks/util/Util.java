	...
	public static void loadCampfireSpawnsFromWorld(Level level) {
		...
		for (File listOfFile : listOfFiles) {
			...
			if (filename.endsWith(".spawnpoint")) {
				...
				String filecontent;
				try {
					filecontent = new String(Files.readAllBytes(Paths.get(campfirespawnfolder + File.separator + filename)));
				} catch (IOException e) {
		...
		for (String fromplayername : fromplayernames) {
			if (CampfireEvent.playercampfires.containsKey(fromplayername)) {
				...
				if (ConfigHandler.sendMessageOnCampfireSpawnUnset) {
					...
					MessageFunctions.sendTranslatableMessage(target, "collective.campfirespawnandtweaks.message.campfirespawnpointremoved", ChatFormatting.DARK_GRAY);
				}
			}
		}
